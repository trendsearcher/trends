...
''' 8 измерений оценки тренда'''
''' float число =   прибыльность сделки (tp/sl) стоп лосс по опыту'''
''' float число =   прибыльность сделки (tp/sl) стоп лосс формальный'''
''' итого 11 оценок на 1 тренд '''
pic2scale = 10 #регулирует масштаб второй картинки в итерации
...
with open (trendspath , "r") as csvfile:
    ...
    for i in my_list0:
        ...
        perekos_sl = ((last_dot - sl_dot[0])/(height_of_trend/2))# <1
        ...
        if I > Imin and length > length_min and perekos_sl < 1:
            if (direction_of_deal == 1 and angle <= 0) or (direction_of_deal == 2 and angle >=0):
                counter_list.append([first_dot, last_dot, angle, b_coeff, sl_dot, sl2_dot])
...
while i <= max_counter:
    ...
    i+= 1
    #####################################
#    var = input("1.2.2.1.2.2..... - оценки \\ число - стираем ошибку \\ Nan - записываем // '--' пропускаем: ")
#    
#    if len(var) == 21:
