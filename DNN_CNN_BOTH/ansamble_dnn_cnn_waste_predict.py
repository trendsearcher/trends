...
###############
def get_list():
    with open('C:\\Users\\user_PC\\Desktop\\sber2\\samples_rating1.csv', newline='') as csvfile:
...
#    Y_test= Y_set_pic.loc[list_of_test_index].values.tolist()
    Y_train = [get_list()[x] for x in list_of_train_index]
    Y_test= [get_list()[x] for x in list_of_test_index]
    ###############################################################################
...
#    X_test = X_test.reshape(len(X_test),28,1)     
    ...
    sumgrades.append(similarity(matrix, Y_test)/156)
