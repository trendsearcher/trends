...
#######____II_____#########
    ...
    zazor = 0.018 
    proboy = 0.05 # зазор выхода из канала
    peakgrad = 0.08# острота пиков на большом отрезке 0,0141 0.0212 0.0282
    window_back2 = 52
if stage == 3:
######____III_____#########
    step_read = 156
    ...
    zazor = 0.00078 
    proboy = 0.00156 # зазор выхода из канала
    peakgrad = 0.00195# острота пиков на большом отрезке 0,0141 0.0212 0.0282
    window_back = 624
if stage == 4:
######____IV_____#########
    step_read = 481
    ...
    zazor = 0.0024 
    proboy = 0.0048 # зазор выхода из канала
    peakgrad = 0.0060# острота пиков на большом отрезке 0,0141 0.0212 0.0282
    window_back = 1926
##############################################################################
##########################################################################
counter_list = []
    ...
    for i in my_list0:
        ...
        if I > 5 and leng > 600: # отбрасываем всякую шнягу
            if (direction_of_deal == 1 and angle <= 0) or (direction_of_deal == 2 and angle >=0):
                counter_list.append([first_dot, last_dot, angle, b_coeff, i, direction_of_deal])
for i in counter_list[100:250]:
    direction = i[-1]
    start_dot_2 = i[0]
    end_dot_2 = i[1]
    angle_2 = i[2]
